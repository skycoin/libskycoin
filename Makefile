...
SWAGGER_DIR = lib/swagger

# Source files
LIB_FILES = $(shell find $(LIBSRC_DIR) -type f -name "*.go")
HEADER_FILES = $(shell find $(INCLUDE_DIR) -type f -name "*.h")
SWAGGER_FILES += skycoin.openapi.v2.json
SWAGGER_FILES += skycoin.openapi.v2.yml
...
update-swagger2: install-deps-swagger ## Generate swagger 2.0 specification for Skycoin API
	@rm -rf $(SWAGGER_DIR)/*.openapi.v2.yml
	@rm -rf $(SWAGGER_DIR)/*.openapi.v2.json
	@echo "Start swagger spec generation"
	-swagger generate spec -b github.com/skycoin/skycoin/src/api -o $(LIBSRC_ABS_PATH)/$(SWAGGER_DIR)/skycoin.openapi.v2.yml
	-swagger generate spec -b github.com/skycoin/skycoin/src/api -o $(LIBSRC_ABS_PATH)/$(SWAGGER_DIR)/skycoin.openapi.v2.json
	@echo "Finished swagger spec generation"

generate-c-client: ## Generate C wrapper for Skycoin Api
	@rm -rf ./lib/Clients/cClient
	# TODO, if you are in another git branch, set git rev-parse branch to the branch you want
	@openapi-generator generate -g c --additional-properties=prependFormOrBodyParameters=true -o ./lib/Clients/cClient$(git rev-parse stdevMac_t3_wrapper_skycoin_api) -i ./lib/swagger/swagger.yml
	@echo "Finished C Wrapper generation"

check-swagger2: update-swagger2 ## Validate the specification generated by go-swagger against swagger 2.0 specification
	@echo "Start swagger validation."
	$(shell echo '$(SWAGGER_FILES)' | tr ' ' '\n' | while read NAME; do swagger validate "$(LIBSRC_ABS_PATH)/$(SWAGGER_DIR)/$$NAME" ; done )
	@echo "Finished swagger validation."
...
check: lint test-libc check-swagger2 ## Run tests and linters
