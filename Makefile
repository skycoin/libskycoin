...
swagger2: install-deps-swagger ## Generate swagger 2.0 specification for Skycoin API
	@rm -rf $(SWAGGER_SPEC_DIR)/*.openapi.v2.yml
	@rm -rf $(SWAGGER_SPEC_DIR)/*.openapi.v2.json
	@echo "Start swagger spec generation"
	-swagger generate spec -b github.com/skycoin/skycoin/src/api -o $(LIBSRC_ABS_PATH)/$(SWAGGER_SPEC_DIR)/skycoin.openapi.v2.yml
	-swagger generate spec -b github.com/skycoin/skycoin/src/api -o $(LIBSRC_ABS_PATH)/$(SWAGGER_SPEC_DIR)/skycoin.openapi.v2.json
	@echo "Finished swagger spec generation"

swagger-client: ## Generate C client for accessing Skycoin REST API
	@rm -rf $(LIBSRC_ABS_PATH)/$(SWAGGER_CLIENT_DIR)/c
	@openapi-generator generate -g c --additional-properties=prependFormOrBodyParameters=true -o $(LIBSRC_ABS_PATH)/$(SWAGGER_CLIENT_DIR)/c -i $(LIBSRC_ABS_PATH)/$(SWAGGER_SPEC_DIR)/skycoin.openapi.v2.yml
	@echo "Finished C Wrapper generation"

check-swagger2: swagger2 ## Validate the specification generated by go-swagger against swagger 2.0 specification
	@echo "Start swagger validation."
	$(shell echo '$(SWAGGER_FILES)' | tr ' ' '\n' | while read NAME; do swagger validate "$(LIBSRC_ABS_PATH)/$(SWAGGER_SPEC_DIR)/$$NAME" ; done )
...
check: lint test-libc f2 ## Run tests and linters
