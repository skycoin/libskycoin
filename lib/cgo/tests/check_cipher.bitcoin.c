...
START_TEST(TestBitcoinWIFRoundTrip)
{
  ...
  ck_assert(isGoStringEq(&(*(GoString *)&seckeyhex1), &(*(GoString *)&seckeyhex2)));
  ck_assert(isGoStringEq(&(*(GoString *)&wip1), &(*(GoString *)&wip2)));
}
END_TEST
...
START_TEST(TestBitcoinWIF ){
 ...
 for (int i = 0; i < 3; i++)
 {
   ...
   ck_assert(isGoStringEq( &(*(GoString*)&string),&(*(GoString*)&pub[i]) ));
