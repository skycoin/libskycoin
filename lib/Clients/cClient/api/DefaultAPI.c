...
DefaultAPI_addressCount(apiClient_t *apiClient) {
    ...
    object_t *elementToReturn = object_parseFromJSON(apiClient->dataReceived);
...
DefaultAPI_outputsGet(apiClient_t *apiClient, list_t* address, list_t* hash) {
    ...
    {
    //listContainer
    localVarQueryParameters = address;
    }
...
DefaultAPI_walletNewSeed(apiClient_t *apiClient, char* entropy) {
    ...
    valueQuery_entropy = strdup(entropy);
    keyPairQuery_entropy = keyValuePair_create(keyQuery_entropy, valueQuery_entropy);
