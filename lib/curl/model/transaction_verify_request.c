...
transaction_verify_request_t *transaction_verify_request_create(
	...
	transaction_verify_request_local_var->unsigned = unsigned;
...
cJSON *transaction_verify_request_convertToJSON(transaction_verify_request_t *transaction_verify_request) {
    ...
    if(transaction_verify_request->unsigned) { 
    if(cJSON_AddBoolToObject(item, "unsigned", transaction_verify_request->unsigned) == NULL) {
    goto fail; //Bool
    }
     } 
...
transaction_verify_request_t *transaction_verify_request_parseFromJSON(cJSON *transaction_verify_requestJSON){
    ...
    // transaction_verify_request->unsigned
    cJSON *unsigned = cJSON_GetObjectItemCaseSensitive(transaction_verify_requestJSON, "unsigned");
    if (unsigned) { 
    if(!cJSON_IsBool(unsigned))
    {
    goto end; //Bool
    }
    }
